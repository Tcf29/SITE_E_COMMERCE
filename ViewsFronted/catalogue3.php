<?php
$request=curl_init("http://localhost/E_COMMERCE/ADMIN/CONTROLLER/produitController.php?action=readAllProduitCategorie&id_categorie=3");
...
<section>
          <div class="tv-audio">
            <div class="text-left fs-25 pt-5 ps-5 text-white gap-45">
              <h1>Le Best of de l'audiovisuel et de l'audio</h1>
              <h2>Ici nous proposons une gamme élargie d'ecrans plats<br>
              et d'appareils audio de qualité </h2>
              <i class="fa-solid fa-arrow-down fleche"></i>
            </div>
          </div>
            <?php if(sizeof($tableSousCategorie)>0):?>
             <?php  if(isset($tableSousCategorie["TV LED"])) : ?>
                
        <div class="section-card my-5 p-4">
            <h2 class="text-center mb-4">Télévisions</h2>
            <div class="d-flex mt-5 cards">
                <!-- code php pour automatiser -->
                <?php foreach($tableSousCategorie["TV LED"] as $product):?>
                <form class="card py-3" method="POST" action="ADMIN/CONTROLLER/produitController.php?action=ajoutpanier&id_categorie=3&recherche=false">
            ...
            <div class="section-card my-5 p-3 h-50">
                ...
                <!-- code php pour automatiser -->
             <?php  if(isset($tableSousCategorie["AUDIO"])) : ?>

                <div class="d-flex mt-5 cards">
                 <?php foreach($tableSousCategorie["AUDIO"] as $product):?>
                    <form class="card" method="POST" action="ADMIN/CONTROLLER/produitController.php?action=ajoutpanier&id_categorie=3&recherche=false">
                        <img src="ADMIN/FILES/<?=$product->photo?>" class="promo-img" alt="Promo 1">
                        <h5><?=$product->nom?></h5>
                        <?php if (!isset($_SESSION["object-produit"])){
                         $_SESSION["object-produit"]=[];
