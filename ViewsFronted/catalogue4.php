<?php
$request=curl_init("http://localhost/E_COMMERCE/ADMIN/CONTROLLER/produitController.php?action=readAllProduitCategorie&id_categorie=4");
...
<section>
          <div class="electromenager fs-sm-1 fs-md-2 fs-lg-3 pt-5 position-relative">
            <div class="text-left fs-25 pt-5 ps-5 text-white gap-45 position-absolute top-50-1">
              <h1>Equipez votre maison aussi !</h1>
              <h2>Tout ce qui concerne l'electromenager à votre disposition</h2>
        ...
        <?php if(sizeof($tableSousCategorie)>0):?>
             <?php  if(isset($tableSousCategorie["REFRIGERATEURS"])) : ?>
                
        <div class="section-card my-5 p-4">
            <h2 class="text-center mb-4">Réfrigérateurs</h2>
            <div class="d-flex mt-5 cards">
                <!-- code php pour automatiser -->
                <?php foreach($tableSousCategorie["REFRIGERATEURS"] as $product):?>
                <form class="card py-3" method="POST" action="ADMIN/CONTROLLER/produitController.php?action=ajoutpanier&id_categorie=4&recherche=false">
            ...
            <div class="section-card my-5 p-3 h-50">
                <h2 class="text-center mb-4">Ventillateurs</h2>
                <!-- code php pour automatiser -->
             <?php  if(isset($tableSousCategorie["VENTILLATEURS"])) : ?>

                <div class="d-flex mt-5 cards">
                 <?php foreach($tableSousCategorie["VENTILLATEURS"] as $product):?>
                    <form class="card" method="POST" action="ADMIN/CONTROLLER/produitController.php?action=ajoutpanier&id_categorie=4&recherche=false">
                        <img src="ADMIN/FILES/<?=$product->photo?>" class="promo-img" alt="Promo 1">
                        <h5><?=$product->nom?></h5>
                        <?php if (!isset($_SESSION["object-produit"])){
                         $_SESSION["object-produit"]=[];
